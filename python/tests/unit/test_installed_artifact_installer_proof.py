...
def test_installed_agent_docs_readme_uses_package_resource(tmp_path: Path, monkeypatch) -> None:
    module = _load_proof_module()
    installed_readme = tmp_path / "site-packages" / "databricks_agent_notebooks" / "for_agents" / "README.md"
    installed_readme.parent.mkdir(parents=True)
    installed_readme.write_text("# Agent Notebook Guide\n", encoding="utf-8")

    class FakeResource:
        def joinpath(self, *parts: str) -> Path:
            assert parts == ("for_agents", "README.md")
            return installed_readme

    monkeypatch.setattr(module.resources, "files", lambda package: FakeResource())

    assert module._installed_agent_docs_readme() == installed_readme.resolve()


def test_installed_agent_docs_tree_lists_recursive_files(tmp_path: Path, monkeypatch) -> None:
    module = _load_proof_module()
    package_root = tmp_path / "site-packages" / "databricks_agent_notebooks"
    docs_root = package_root / "for_agents"
    (docs_root / "examples" / "smoke").mkdir(parents=True)
    (docs_root / "README.md").write_text("# Agent Notebook Guide\n", encoding="utf-8")
    (docs_root / "examples" / "smoke" / "python_select_one.md").write_text("# smoke\n", encoding="utf-8")
    ...
    assert module._installed_agent_docs_tree() == docs_root.resolve()
    files = module._installed_agent_docs_files()
    assert "README.md" in files
    assert any(path.startswith("examples/") for path in files)
