...
def _installed_agent_docs_readme() -> Path | None:
    try:
...
def _installed_agent_docs_tree() -> Path | None:
    try:
        resource = resources.files("databricks_agent_notebooks").joinpath("for_agents")
    except (ModuleNotFoundError, FileNotFoundError):
        return None
...
def _installed_agent_docs_files() -> list[str]:
    docs_root = _installed_agent_docs_tree()
...
def run_proof() -> None:
    with tempfile.TemporaryDirectory(prefix="installed-artifact-proof-") as temp_dir:
        ...
        agent_docs_readme = _installed_agent_docs_readme()
        agent_docs_tree = _installed_agent_docs_tree()
        agent_docs_files = _installed_agent_docs_files()
        python_run_input = write_python_run_smoke_input(layout.root)

        _expect(agent_docs_readme is not None, "installed agent docs README is missing")
        _expect(agent_docs_readme.is_file(), f"installed agent docs README missing on disk: {agent_docs_readme}")
        _expect(agent_docs_tree is not None, "installed agent docs tree is missing")
        _expect(agent_docs_tree.is_dir(), f"installed agent docs tree missing on disk: {agent_docs_tree}")
        _expect("README.md" in agent_docs_files, "installed agent docs tree missing README.md")
        _expect(
            any(path.startswith("examples/") for path in agent_docs_files),
            "installed agent docs tree missing packaged examples",
        )
        _expect(
            "# Agent Notebook Guide" in agent_docs_readme.read_text(encoding="utf-8"),
            "installed agent docs README missing expected heading",
        )

        help_result = _run([str(agent_notebook), "help"], env=env, cwd=layout.root)
        _assert_success(help_result, "agent-notebook help")
        _expect(str(agent_docs_readme) in help_result.stdout, "agent-notebook help did not report installed agent docs path")

        long_help_result = _run([str(agent_notebook), "--help"], env=env, cwd=layout.root)
        _assert_success(long_help_result, "agent-notebook --help")
        _expect(str(agent_docs_readme) in long_help_result.stdout, "agent-notebook --help did not report installed agent docs path")
        ...
        _assert_success(doctor_result, "agent-notebook doctor")
        _expect(
            "kernel semantics verified via launcher contract" in doctor_result.stdout,
            "doctor did not report launcher-contract-backed verification",
        )
        _expect("materialized runtime" in doctor_result.stdout, "doctor did not report a materialized runtime")

        post_doctor_snapshot = _snapshot_tree(layout.root)
        _expect(post_doctor_snapshot == pre_doctor_snapshot, "doctor command mutated proof-root artifacts")
