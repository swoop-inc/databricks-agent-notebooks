...
KERNEL_ID_213 = "scala213-dbr-connect"
KERNEL_DISPLAY_NAME_213 = "Scala 2.13 (Databricks Connect)"
...
def install_kernel(
    ...
    *,
    ...
    ----------
    scala_version:
        Scala version for the Almond kernel (``"2.12"`` or ``"2.13"``).
        Controls the ``--scala`` flag passed to coursier.  When callers
        use the default *kernel_id* / *display_name* values, they are
        automatically adjusted to match the variant.
    """
    from databricks_agent_notebooks._constants import SCALA_VARIANTS

    variant = SCALA_VARIANTS.get(scala_version)
    # When caller uses the 2.12 defaults but requests 2.13, swap to 2.13 defaults.
    if variant is not None:
        if kernel_id == KERNEL_ID and scala_version != "2.12":
            kernel_id = variant.kernel_id
        if display_name == KERNEL_DISPLAY_NAME and scala_version != "2.12":
            display_name = variant.kernel_display_name
    ...
    if contract_path is None:
        ...
        try:
            ...
            if not contract.bootstrap_argv:
                ...
                launcher_bin = contract.bootstrap_argv[0]
                launcher_bin_path = Path(launcher_bin)
                if launcher_bin_path.is_absolute():
                    if not launcher_bin_path.is_file():
                        issues.append(f"bootstrap launcher binary not found: {launcher_bin}")
                elif shutil.which(launcher_bin) is None:
                    issues.append(f"bootstrap launcher binary not on PATH: {launcher_bin}")

                for arg in contract.bootstrap_argv:
                    if arg.endswith(".jar") and not Path(arg).is_file():
                        issues.append(f"bootstrap JAR not found: {arg}")
