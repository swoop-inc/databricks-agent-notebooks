...
def test_install_kernel_scala_213(tmp_path: Path) -> None:
    from databricks_agent_notebooks.runtime.kernel import (
    ...
    home = _make_runtime_home(tmp_path / "runtime-home")
    kernel_dir = home.kernels_dir / KERNEL_ID_213
    ...
    def fake_run(*_args, **_kwargs) -> None:
        ...
        (kernel_dir / "kernel.json").write_text(
            json.dumps(
                {
                    "argv": ["/usr/bin/java", "coursier", "--connection-file", "{connection_file}"],
                    "display_name": KERNEL_DISPLAY_NAME_213,
                    "language": "scala",
                    "env": {"SPARK_HOME": "/opt/spark"},
                }
            ),
    ...
    with (
        ...
        result_dir = install_kernel(scala_version="2.13")

    assert result_dir == kernel_dir
    run.assert_called_once_with(
        [
            "/opt/bin/coursier",
            "launch",
...
def test_verify_kernel_recovers_runtime_receipt_path_from_kernel_receipt_when_contract_is_legacy(
    tmp_path: Path,
) -> None:
        ...
        json.dumps(
            {
                ...
                "databricks_line": "16.4",
                "python_line": "3.12",
                "install_root": str(tmp_path / "elsewhere" / runtime_id),
                "installed_at": "2026-03-22T00:00:00+00:00",
                "status": "materialized",
            }
        ),
    ...
    assert "runtime receipt install_root does not match runtime directory" in issues
...
def _setup_valid_kernel(
    tmp_path: Path,
    *,
    bootstrap_argv: list[str] | None = None,
) -> tuple[Path, str]:
    """Create a fully valid kernel setup and return (kernels_dir, kernel_id).

    If *bootstrap_argv* is provided it replaces the default bootstrap argv
...
def test_verify_kernel_bootstrap_launcher_binary_absolute_missing(tmp_path: Path) -> None:
    from databricks_agent_notebooks.runtime.kernel import ADD_OPENS_FLAG, verify_kernel

    kernels_dir, kernel_id = _setup_valid_kernel(
        tmp_path,
        bootstrap_argv=[
            "/nonexistent/java",
            ADD_OPENS_FLAG,
    ...
    issues = verify_kernel(kernels_dir, kernel_id=kernel_id)

    assert any("bootstrap launcher binary not found: /nonexistent/java" in i for i in issues)


def test_verify_kernel_bootstrap_launcher_binary_not_on_path(tmp_path: Path) -> None:
    from databricks_agent_notebooks.runtime.kernel import ADD_OPENS_FLAG, verify_kernel
    ...
    with patch("databricks_agent_notebooks.runtime.kernel.shutil.which", return_value=None):
        issues = verify_kernel(kernels_dir, kernel_id=kernel_id)

    assert any("bootstrap launcher binary not on PATH: nonexistent-java-binary" in i for i in issues)


def test_verify_kernel_bootstrap_launcher_binary_ok(tmp_path: Path) -> None:
    from databricks_agent_notebooks.runtime.kernel import ADD_OPENS_FLAG, verify_kernel
    ...
    with patch("databricks_agent_notebooks.runtime.kernel.shutil.which", return_value="/usr/bin/java"):
        issues = verify_kernel(kernels_dir, kernel_id=kernel_id)

    assert not any("bootstrap launcher binary" in i for i in issues)


def test_verify_kernel_bootstrap_jar_missing(tmp_path: Path) -> None:
    from databricks_agent_notebooks.runtime.kernel import ADD_OPENS_FLAG, verify_kernel
    ...
    assert any("bootstrap JAR not found: /nonexistent/path/almond.jar" in i for i in issues)
